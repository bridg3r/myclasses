...
Do Meshed or Before Methods have any positive benefit on memory recall when compared to control groups? Which method is better? To answer these questions we must run three statistical tests. Our questions can be formally written as follows when comparing each of the three groups to each other.
...
We will check the distribution of the data:

```{r, message=FALSE, warning=FALSE}
ggplot(Friendly, aes(x=correct)) +
facet_wrap(~condition) +
geom_histogram() +
labs(title = "Compare medians without filter")
```
We see that none of the groups are normally distributed. We also know that each group is only a sample size of n = 10 so we the requirements are not met for a regular Independent Samples t-Test. Because of this, we must result to the non-parametric equivalent a Wilcoxon Rank Sum Test.

### Analysis

#### Control Group vs. Before
```{r, message=FALSE, warning=FALSE}
pander(wilcox.test(Friendly$correct[Friendly$condition == "SFR"], Friendly$correct[Friendly$condition == "Before"], mu = 0, alternative = "less", conf.level = 0.95))    
```
We see that the p-value of .02278 comes out to be less than our alpha of .05 here so we can safely reject the null hypothesis that their medians are the same and assume that our alternative hypothesis is correct that our Before Test tends to be more effective than compared to the randomized learning done in the control group.


#### Control Group vs. Meshed
```{r, message=FALSE, warning=FALSE}
pander(wilcox.test(Friendly$correct[Friendly$condition == "SFR"], Friendly$correct[Friendly$condition == "Meshed"], mu = 0, alternative = "less", conf.level = 0.95))    
```
We see that the p-value of .05075 comes out to be less than our alpha of .05 here so we can safely reject the null hypothesis that their medians are the same and assume that our alternative hypothesis is correct that our Meshed Test tends to be more effective than compared to the randomized learning done in the control group.
...
pander(favstats(correct ~ condition, data=Friendly)[-10])


```
Here we compare the two tests to each other and come up with a p-value of .378 which is much greater than our alpha so we fail to reject our null hypothesis that their true location shift is equal to 0, or that they yield the same results. There is not enough evidence to conclude that one is better than the other.

#### Conclusion
To sum up our results, we found that both the Before method and Meshed method yielded better results for the memory game when compared to the control group. When we compared them to each other however, there was no evidence to say that their was a significant difference in results. As far as we know now, anyone is equally well-off with either method when attempting to improve their results with the memory game or any other information that one wishes to learn.
