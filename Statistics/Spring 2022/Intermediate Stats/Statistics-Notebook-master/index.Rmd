...
## Table of Contents 
Statistics is everything awesome!!!!!
...
<!-- The t test is what is done to get an accurate estimate of the population. Independent t tests is good for comparing two groups. Paired t tests is good for comparing a subject before and after some sort of affect.  --------------- -->
...
KidsFeet %>% 
  group_by(sex) %>% 
  summarise(min = min(length), Q1=quantile(length,0.25), median=median(length),
            Q1=quantile(length,0.25), median=median(length),Q3=quantile(length,0.75),
            max=max(length))
